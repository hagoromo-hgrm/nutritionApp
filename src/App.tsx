...
function MenuNutritionDetailsModal({ menu, menus, foods, goals, onClose }: { menu: Menu; menus: Menu[]; foods: Food[]; goals: NutritionGoals; onClose: () => void }) {
  ...
  return <div className="modal-backdrop nutrient-detail-backdrop" role="dialog" aria-modal="true" aria-label={`${menu.name}の栄養詳細`}><section className="modal-card nutrient-detail-modal menu-nutrition-details-modal"><div className="modal-heading"><div><span className="eyebrow">MY MENU NUTRIENTS</span><h2>{menu.name}</h2><p className="muted">登録済みの構成食材1食分です。</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="閉じる">×</button></div><div className="detail-total"><span>1食分のカロリー</span><strong>{formatNutrient(menuFood.nutrients.energyKcal)}<small> kcal</small></strong></div><NutrientGoalGraphs nutrients={menuFood.nutrients} goals={goals} /></section></div>
}
