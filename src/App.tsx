...
function TodayDetailsModal({ total, goals, entries, subtotals, onClose }: { total: Nutrients; goals: NutritionGoals; entries: MealEntry[]; subtotals: Record<string, Nutrients>; onClose: () => void }) {
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="今日の栄養詳細"><section className="modal-card today-details-modal"><div className="modal-heading"><div><span className="eyebrow">TODAY DETAILS</span><h2>今日の詳細</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="閉じる">×</button></div><NutrientGoalGraphs nutrients={total} goals={goals} subtotals={subtotals} colorByMeal /><div className="today-detail-count">食事記録 {entries.length}件</div><button className="button ghost full-width" type="button" onClick={onClose}>閉じる</button></section></div>
}
...
function MenuFoodSelection({ selectedIds, foods, recentFoods, favoriteFoods, favoriteIds, onToggleFavorite, onAdd, onRemove }: MenuFoodSelectionProps) {
  ...
  const visibleFoods = (normalizedQuery ? foods.filter((food) => displayFoodName(food).toLocaleLowerCase('ja-JP').includes(normalizedQuery)) : foods).slice(0, 60)
