...
function NavButton({ active, onClick, icon, iconClass, label }: NavButtonProps) {
  return <button type="button" className={`nav-item${active ? ' active' : ''}`} onClick={onClick}><span className={iconClass}>{icon === 'menu-grid' ? <span className="menu-grid-table" aria-hidden="true"><i /><i /><i /><i /></span> : icon === 'settings' ? <img className="settings-nav-icon" src={SETTINGS_ICON_ASSET} alt="" aria-hidden="true" /> : icon}</span>{label}</button>
}
