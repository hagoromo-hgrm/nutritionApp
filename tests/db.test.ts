...
describe('IndexedDB data safety', () => {
  it('MEXTの100g基準と食品ごとの入力単位初期値を分離する', async () => {
    ...
    const seasoning = await db.foods.get('mext_17007')
    for (const food of [rice, riceGrain, egg, banana, tofu, seasoning]) {
      expect(food?.baseAmount).toBe(100)
      expect(food?.baseUnit).toBe('g')
    }
    expect(rice?.inputUnitConversions).toEqual([{ unit: '杯', baseAmount: 140 }])
    expect(rice?.servingAmount).toBe(1)
    expect(rice?.servingUnit).toBe('杯')
    expect(riceGrain?.inputUnitConversions).toEqual([{ unit: '合', baseAmount: 150 }])
    expect(egg?.inputUnitConversions).toEqual([{ unit: '個', baseAmount: 50 }])
    expect(banana?.inputUnitConversions).toEqual([{ unit: '本', baseAmount: 100 }])
    expect(tofu?.inputUnitConversions).toEqual([{ unit: '丁', baseAmount: 300 }])
    expect(seasoning?.inputUnitConversions).toEqual([{ unit: '小さじ', baseAmount: 6 }])
  })

  it('食品名の一部分だけが似た別食品へ入力単位を誤適用しない', async () => {
    expect((await db.foods.get('mext_02006'))?.inputUnitConversions).toEqual([{ unit: '本', baseAmount: 250 }])
    expect((await db.foods.get('mext_11005'))?.inputUnitConversions).toEqual([])
    expect((await db.foods.get('mext_10438'))?.inputUnitConversions).toEqual([])
    expect((await db.foods.get('mext_07030'))?.inputUnitConversions).toEqual([])
    expect((await db.foods.get('mext_07154'))?.inputUnitConversions).toEqual([{ unit: '小さじ', baseAmount: 7 }])
    expect((await db.foods.get('mext_07108'))?.inputUnitConversions).toEqual([])
    expect((await db.foods.get('mext_12014'))?.inputUnitConversions).toEqual([])
  })
  ...
  it('MEXTの収集済み食品を初期データとして検索できる', async () => {
    ...
    expect((await db.foods.get('mext_01088'))?.nutrients.saturatedFatG).toBe(0.1)
    expect((await db.foods.get('mext_12004'))?.nutrients.saturatedFatG).toBe(3.12)
    ...
    expect(rice?.baseUnit).toBe('g')
    expect(seasoning?.baseAmount).toBe(100)
    expect(seasoning?.baseUnit).toBe('g')
    expect(seasoning?.name).not.toContain('（小さじ1=')
    expect((await db.foods.where('source').equals('mext').toArray()).every((food) => food.baseAmount === 100 && food.baseUnit === 'g')).toBe(true)
